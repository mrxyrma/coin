...
/**
 * Высота рельефа над полем монеты.
 *
 * Подобрана так, чтобы объём читался на силуэте, когда монета
 * поворачивается ребром, — в этом весь смысл Ultra High Relief.
 * При 0.055 (половина толщины заготовки) рельеф на просвет пропадал.
 */
const RELIEF = 0.115
...
function obverseHeight(r, theta) {
  ...
  // К центру гасим — там радиальные линии сходятся в кашу и забивают купол
  const guilloche =
    Math.sin(r * 130) * 0.5 + Math.sin(theta * 26 + r * 40) * 0.5
  h += guilloche * 0.05 * band(r, 0.34, 0.86, 0.07)
