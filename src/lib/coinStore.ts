...
export const coinUi = {
  rotY: 0,
  /**
   * Счётчик «сцена изменилась не из-за скролла».
   *
   * Кадр рисуется только при изменении сигнатуры (см. CoinCanvas), поэтому
   * события вроде «догрузилась модель» нужно объявлять явно — иначе на
   * неподвижной странице новая геометрия так и не появится на экране.
   */
  nonce: 0,
}

export const invalidateCoin = () => {
  coinUi.nonce++
}
